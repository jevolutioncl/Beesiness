...
function filterUsers() {
    ...
    fetch(`/Tarea/TareaIndex?searchString=${searchString}&filterType=${currentFilterType}`)
...
//comentados todos los metodos de paginacion
/*
function goToPage(pageNumber) {
    const searchString = document.getElementById('searchString').value;
    fetch(`/Tarea/TareaIndex?searchString=${searchString}&filterType=${currentFilterType}&pageNumber=${pageNumber}`)
        .then(response => response.text())
        .then(html => {
            const doc = new DOMParser().parseFromString(html, "text/html");
            document.querySelector('.table').innerHTML = doc.querySelector('.table').innerHTML;
