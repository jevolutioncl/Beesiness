...
function filterUsers() {
    ...
    fetch(`/Enfermedad/EnfermedadIndex?searchString=${searchString}&filterType=${currentFilterType}`)
...
function goToPage(pageNumber) {
    const searchString = document.getElementById('searchString').value;
    fetch(`/Enfermedad/EnfermedadIndex?searchString=${searchString}&filterType=${currentFilterType}&pageNumber=${pageNumber}`)
        .then(response => response.text())
        .then(html => {
            const doc = new DOMParser().parseFromString(html, "text/html");
            document.querySelector('.table').innerHTML = doc.querySelector('.table').innerHTML;
